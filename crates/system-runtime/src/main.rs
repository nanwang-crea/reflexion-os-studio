...
fn handle_file_read(params: Value) -> Result<Value, OpError> {
    ...
    let result =
        files::read(&root, &params.path).map_err(|message| OpError::new("file_error", message))?;
    Ok(json!({ "content": result.content, "sizeBytes": result.size_bytes }))
}
...
fn handle_file_list(params: Value) -> Result<Value, OpError> {
    ...
    let root = workspace_root(&params.workspace_root)?;
    let entries =
        files::list(&root, &params.path).map_err(|message| OpError::new("file_error", message))?;

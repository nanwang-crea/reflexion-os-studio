...
- **模型窗口元数据 ✅(2026-08-31)**:ProviderProfile 增加 `contextWindow`(设置页可配,迁移 v9);`contextBudgetFor` 按 `min(24k, 窗口×0.75 − maxTokens 预留)` 动态取预算(下限 1024),同时作用于启动压缩与轮内裁剪——未配置窗口回退保守默认。
